/**
 * API Key Authentication Handler for Wolfram Knowledge Engine
 *
 ...
 * 6. Return response
 *
 * When you add new tools to server.ts, you MUST also update:
 * 1. Register them in getOrCreateServer()
 * 2. Add tool executor functions
 * 3. Add cases to handleToolsCall()
 * 4. Add tool schemas to handleToolsList()
 */
 ...
 *
 * When you add new tools to server.ts, you MUST add them here too!
 *
 * @param env - Cloudflare Workers environment
...
async function getOrCreateServer(
  ...
  const server = new McpServer({
    name: "Wolfram Knowledge Engine (API Key)",
    version: "1.0.0",
  });

  const apiClient = new WolframApiClient(env);
  ...
  server.tool(
    ...
    async ({ query, units }) => {
      ...
      try {
        // Step 2: Check operational cache
        const cacheKey = WolframApiClient.getCacheKey(TOOL_NAME, query + (units || ""));
        const cachedResult = await WolframApiClient.checkCache(env.CACHE_KV, cacheKey);

        if (cachedResult) {
          return {
            content: [{ type: "text" as const, text: `[Cached Result]\n\n${cachedResult}` }],
          };
        }
        ...
        await WolframApiClient.storeCache(env.CACHE_KV, cacheKey, finalResult);
 ...
 *
 * When you add new tools, update this list to match!
 */
      ...
      inputSchema: {
        ...
        properties: {
          query: {
            type: "string",
            minLength: 1,
            description: "Natural language query (e.g., 'How far is LA from NY?')",
          },
          units: {
            type: "string",
            enum: ["metric", "imperial"],
            description: "Unit system (optional, default: metric)",
          },
        },
        required: ["query"],
      },
      ...
      inputSchema: {
        ...
        properties: {
          query: {
            type: "string",
            minLength: 1,
            description: "Complex query requiring detailed analysis",
          },
          maxchars: {
            type: "number",
            minimum: 1000,
            maximum: 10000,
            description: "Max character limit (default: 6800, max: 10000)",
          },
        },
        required: ["query"],
      },
 ...
 *
 * When you add new tools, add cases to the switch statement!
 */
async function handleToolsCall(
  ...
  try {
    ...
    switch (toolName) {
      case "getQuickAnswer":
        result = await executeGetQuickAnswerTool(toolArgs, env, userId);
        break;

      case "getDetailedAnalysis":
        result = await executeGetDetailedAnalysisTool(toolArgs, env, userId);
        break;
...
/**
 * Execute getQuickAnswer tool
 * Get quick answers from WolframAlpha Short Answers API
 */
async function executeGetQuickAnswerTool(
  ...
  try {
    ...
    // Step 2: Check operational cache
    const cacheKey = WolframApiClient.getCacheKey(TOOL_NAME, args.query + (args.units || ""));
    const cachedResult = await WolframApiClient.checkCache(env.CACHE_KV, cacheKey);
    ...
    // Step 4: Execute WolframAlpha API call
    const result = await apiClient.getQuickAnswer(args.query, args.units);
    ...
    await consumeTokensWithRetry(
      ...
      "knowledge-engine",
      TOOL_NAME,
      { query: args.query, units: args.units },
      finalResult,
      true,
      actionId
    );
...
/**
 * Execute getDetailedAnalysis tool
 * Get detailed analysis from WolframAlpha LLM API
 */
async function executeGetDetailedAnalysisTool(
  args: Record<string, any>,
  env: Env,
  userId: string
): Promise<any> {
  ...
  try {
    ...
    // Step 2: Check operational cache
    const cacheKey = WolframApiClient.getCacheKey(TOOL_NAME, args.query + (args.maxchars || ""));
    const cachedResult = await WolframApiClient.checkCache(env.CACHE_KV, cacheKey);
    ...
    // Step 4: Execute WolframAlpha API call
    const result = await apiClient.getDetailedAnalysis(args.query, args.maxchars);
    ...
    await consumeTokensWithRetry(
      ...
      "knowledge-engine",
      TOOL_NAME,
      { query: args.query, maxchars: args.maxchars },
      finalResult,
      true,
      actionId
    );
