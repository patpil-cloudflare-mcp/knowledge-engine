...
import { WolframKnowledgeEngineMCP } from "./server";
import { AuthkitHandler } from "./authkit-handler";
import { handleApiKeyRequest } from "./api-key-handler";
import type { Env } from "./types";

// Export the McpAgent class for Cloudflare Workers
export { WolframKnowledgeEngineMCP };

/**
 * Wolfram Knowledge Engine with Dual Authentication Support
 *
 ...
 * - getQuickAnswer: Get quick answers from WolframAlpha Short Answers API (2 tokens)
 * - getDetailedAnalysis: Get comprehensive analysis from WolframAlpha LLM API (4 tokens)
 */
...
const oauthProvider = new OAuthProvider({
    ...
    apiHandlers: {
        '/sse': WolframKnowledgeEngineMCP.serveSSE('/sse'),  // Legacy SSE transport
        '/mcp': WolframKnowledgeEngineMCP.serve('/mcp'),     // New Streamable HTTP transport
    },
